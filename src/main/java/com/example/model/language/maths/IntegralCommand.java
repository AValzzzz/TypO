...
public class IntegralCommand implements Command {
    private static final Pattern BOUNDED = Pattern.compile(
            "^int_(?:\\(([^()]+)\\)|([\\p{L}\\p{N}]+))\\^(?:\\(([^()]+)\\)|([\\p{L}\\p{N}]+))\\((.+)\\)$");
    private static final Pattern SIMPLE = Pattern.compile("^int\\((.+)\\)$");
    ...
    public CommandResult apply(String raw) {
        ...
        if (b.matches()) {
            ...
            return CommandResult.ofMathObject(new MathObject(MathObject.Type.INTEGRAL, rawData));
        }
        Matcher s = SIMPLE.matcher(raw);
        s.matches();
        return CommandResult.ofMathObject(new MathObject(MathObject.Type.INTEGRAL, "|" + "|" + s.group(1)));
    }
}
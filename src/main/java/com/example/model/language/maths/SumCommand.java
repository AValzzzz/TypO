...
public class SumCommand implements Command {
    private static final Pattern BOUNDED = Pattern.compile(
        "^sum_(?:\\(([^()]+)\\)|([\\p{L}\\p{N}]+))\\^(?:\\(([^()]+)\\)|([\\p{L}\\p{N}]+))\\((.+)\\)$"
    );
    private static final Pattern SIMPLE = Pattern.compile("^sum\\((.+)\\)$");
    ...
    public CommandResult apply(String raw) {
        ...
        if (b.matches()) {
            ...
            return CommandResult.ofMathObject(new MathObject(MathObject.Type.SUM, rawData));
        }
        Matcher s = SIMPLE.matcher(raw);
        s.matches();
        return CommandResult.ofMathObject(new MathObject(MathObject.Type.SUM, "|" + "|" + s.group(1)));
    }
}
